...
const fixtureStart = JSON.stringify({
  choices: [
    {
      delta: {
        tool_calls: [
          {
            ...
            id: 'call_1',
            type: 'function',
            function: { name: 'report_smoke_target', arguments: '{"provider":"qwen"' },
          },
        ],
      },
    },
...
const fixtureFinish = JSON.stringify({
  choices: [
    {
      delta: { tool_calls: [{ index: 0, function: { arguments: ',"model":"qwen3"' } }] },
      finish_reason: 'tool_calls',
    },
  ],
})

const fixture = [`data: ${fixtureStart}`, '', `data: ${fixtureFinish}`].join('\n')
