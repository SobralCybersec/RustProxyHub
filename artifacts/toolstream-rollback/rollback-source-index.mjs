...
async function initChatGPT({ runtime_dir, headless, browser, oauth = false }) {
  process.chdir(runtime_dir)
  if (oauth) {
...
async function listChatGPTHybridModels({ chatgpt_mode = 'auto' } = {}) {
  // Model discovery is single-source. `auto` previously fetched both the web
  // catalog and Codex OAuth catalog in one request; callers now toggle mode.
  const mode = chatgpt_mode === 'codex' ? 'codex' : 'web'
  const items = []
  const seen = new Set()
  const push = (id, api) => {
    if (!id || seen.has(id)) return
    seen.add(id)
    items.push({ id, provider: 'chatgpt', api })
  }
  ...
  if (mode !== 'codex') {
    ...
    if (state.chatgpt.cachedHeaders?.model) addModelCandidate(ids, 'chatgpt', state.chatgpt.cachedHeaders.model)
    for (const id of ids) push(id, 'chat_completions')
  }

  if (mode !== 'web' && state.chatgpt.oauth) {
    try {
      const codex = await state.chatgpt.oauth.listModels()
      for (const item of codex?.data || []) push(item?.id, 'codex_responses')
    } catch {}
  }

  return {
    data: items.length ? items : [{ id: mode === 'codex' ? 'gpt-5.4-mini' : 'chatgpt-web-session', provider: 'chatgpt', api: mode === 'codex' ? 'codex_responses' : 'chat_completions' }],
  }
}

async function chatChatGPTWithUi({ model, prompt, system_prompt, web_search = false, fallbackReason = null }) {
  const page = state.chatgpt.page
  if (!page) throw new Error('ChatGPT Playwright not initialized')
  ...
  await page.waitForSelector(inputSelector, { timeout: 30000 }).catch(() => {
    throw new Error('ChatGPT web UI fallback requires an active ChatGPT login.')
  })

  const assistantCount = await page.locator(assistantSelector).count()
  const input = page.locator(inputSelector).first()
  await input.fill(foldChatGPTSystemPrompt(system_prompt, prompt))
  await input.press('Enter')

  const assistant = page.locator(assistantSelector).nth(assistantCount)
  await assistant.waitFor({ state: 'visible', timeout: 90000 }).catch(() => {
    throw new Error('ChatGPT web UI fallback did not start an assistant response.')
  })
  ...
  for (let attempt = 0; attempt < 90; attempt += 1) {
    ...
    if (text) {
      return {
        ...
        warning: [
          'ChatGPT browser API replay returned 401; used active web UI session.',
          web_search ? 'Web search follows the current ChatGPT UI setting.' : null,
          fallbackReason,
        ]
          .filter(Boolean)
          .join(' | '),
  ...
  throw new Error('ChatGPT web UI fallback completed without assistant text.')
}
...
async function chatChatGPTWeb({ model, prompt, system_prompt, web_search = false }) {
  ...
  try {
    ...
    return chatChatGPTWithUi({
      model,
      prompt,
      system_prompt,
      web_search,
      fallbackReason: `Request-template capture failed: ${error instanceof Error ? error.message : String(error)}`,
    })
  }
  ...
  const sendConversation = async (preparedPrompt) => {
    ...
    let requestResult
    try {
      requestResult = await page.evaluate(async ({ headers, payload }) => {
  ...
  const preparedPrompt = compactChatGPTPrompt(prompt, 18000)
  ...
  if (!sent.requestResult.ok && ([401, 403].includes(sent.requestResult.status) || sent.requestResult.status === 0)) {
    return chatChatGPTWithUi({
      ...
      web_search,
      fallbackReason:
        sent.requestResult.status === 0
          ? `Request replay transport error: ${sent.requestResult.transportError || 'unknown error'}`
          : `Request replay returned HTTP ${sent.requestResult.status}`,
    })
  }

  let conversationId = sent.requestResult.conversationId || sent.payload.conversation_id || ''
  if (!sent.requestResult.ok || !conversationId) {
    if ([401, 403].includes(sent.requestResult.status)) {
      throw new Error(`ChatGPT web session unauthorized (HTTP ${sent.requestResult.status}). Open GPT web login and retry.`)
    }
  ...
  const readConversation = async (id) => page.evaluate(async ({ headers, conversationId }) => {
    let lastStatus = 0
    for (let attempt = 0; attempt < 60; attempt += 1) {
      ...
      let response
      try {
        response = await fetch(`https://chatgpt.com/backend-api/conversation/${conversationId}`, {
          method: 'GET',
          credentials: 'include',
          headers,
          signal: controller.signal,
  ...
  }, { headers: requestHeaders, conversationId: id })

  let conversationJson = await readConversation(conversationId)
  if (conversationJson.status === 401 || conversationJson.status === 403) {
    await refreshWebSession()
    sent = await sendConversation(preparedPrompt)
    conversationId = sent.requestResult.conversationId || sent.payload.conversation_id || ''
    if (!sent.requestResult.ok || !conversationId) {
      throw new Error(`ChatGPT web session unauthorized after refresh (HTTP ${sent.requestResult.status || 401})`)
    }
    conversationJson = await readConversation(conversationId)
  }

  if (conversationJson.status === 401 || conversationJson.status === 403) {
    throw new Error(`ChatGPT web session unauthorized (HTTP ${conversationJson.status}). Open GPT web login and retry.`)
  }
  const text = extractChatGPTAssistantText(conversationJson.body ? JSON.parse(conversationJson.body) : null)
  if (!text) {
...
function chatGptModelId(model) {
  return String(model || '').replace(/^chatgpt:/i, '').trim()
}

function isChatGPTWebSessionModel(model) {
  const id = chatGptModelId(model)
  return (
    CHATGPT_WEB_MODEL_IDS.includes(id) ||
    /(?:^chatgpt-web-session$|(?:^|[-_.:])web(?:[-_.:]|$)|(?:^|[-_.:])web-session$)/i.test(id)
  )
}
...
async function openChatGPTLogin({ runtime_dir, browser, oauth = false }) {
  await initChatGPT({ runtime_dir, headless: false, browser, oauth })
  if (oauth) {
    const oauthPage = state.chatgpt.context ? await state.chatgpt.context.newPage().catch(() => null) : null
    if (oauthPage) await state.chatgpt.oauth?.startLogin(oauthPage).catch(() => {})
    return
  }
...
async function handle(method, provider, params) {
  switch (`${provider}:${method}`) {
    ...
    case 'chatgpt:init':
      return initChatGPT({ ...(params || {}), oauth: false })
    ...
    case 'chatgpt:manual_login':
      return openChatGPTLogin({ ...(params || {}), oauth: false })
    ...
    case 'codex:capture_headers':
      return captureChatGPTTemplate(!!params.force_new)
    case 'codex:basic_headers':
      return getChatGPTBasicHeaders()
    case 'codex:manual_login':
      return openChatGPTLogin({ ...(params || {}), oauth: true })
